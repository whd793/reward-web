...
export const MockUsersService = {
  ...
  create: jest.fn().mockImplementation(createUserDto => {
    const newUser = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      toObject: () => ({ ...newUser, password: undefined }),
    };
    return newUser;
  }),
};
...
export const MockAuthService = {
  ...
  login: jest.fn().mockImplementation(async loginUserDto => {
    const user = await MockAuthService.validateUser(loginUserDto.username, loginUserDto.password);
